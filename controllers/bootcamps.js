...
exports.updateBootcamp = asyncHandler( async (req,res,next) => {
        ...
        //* Make sure user is bootcamp owner 
         if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') { //* bootcamp'in kurucusu , şuan online olan kurucu değilse yada rolü admin değilse..
...
exports.deleteBootcamp =  asyncHandler( async (req,res,next) => {
        ...
        //* Make sure user is bootcamp owner 
        if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') { //* bootcamp'in kurucusu , şuan online olan kurucu değilse yada rolü admin değilse..
          return next(`User  ${req.params.id} is not authorized to delete this bootcamp`, 401)
         }
...
exports.bootcampUploadPhoto =  asyncHandler( async (req,res,next) => {
  ...
  file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}` , async err => { //* ilk girdiğimiz path
    if (err) {
      console.error(err)
      return next(new ErrorResponse(`A Problem occured about file upload`, 500))
    }
